...
#include "fs.h"
#include "buf.h"
// #define NBUFMAP_BUCKET 13 //定义哈希桶个数
// //哈希索引
...
//   // struct buf head;
//   struct buf buf[NBUF];//内存中缓存块的数量
//   //哈希表
//   struct buf bufmap[NBUFMAP_BUCKET];//为了快速定位
//   struct spinlock bufmap_locks[NBUFMAP_BUCKET];//桶锁
//   struct spinlock eviction_lock;//驱逐锁
// } bcache;//将大的缓冲区划分位小的缓冲区
...
//     initsleeplock(&b->lock,"buff");
//     b->lastuse = 0;
//     b->refcnt = 0;

//     //将所有缓冲区块添加到bufmap[0] 头插法
...
//     int new_found = 0; //是否在当前桶中找到新的LRU-buf
...
//     else{
//       if(holding_bucket !=-1)
//       release(&bcache.bufmap_locks[holding_bucket]);//找到了不是第一个桶释放桶锁
//        holding_bucket =i;//把标记的 holding_bucket更改成当前桶锁编号
//     }
//   }

//   //如果没有找到任何一个LRU-buf表示没有空闲的缓存块
//   if(!before_least)
